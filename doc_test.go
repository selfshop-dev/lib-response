...
func ExampleWriter_BadRequest() {
	rw := httptest.NewRecorder()
	r := httptest.NewRequestWithContext(context.Background(), http.MethodPost, "/users", nil)

	testWriter.BadRequest(rw, r, "invalid json body")
...
func ExampleWriter_NotFound() {
	rw := httptest.NewRecorder()
	r := httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/users/99", nil)

	testWriter.NotFound(rw, r, "user not found")
